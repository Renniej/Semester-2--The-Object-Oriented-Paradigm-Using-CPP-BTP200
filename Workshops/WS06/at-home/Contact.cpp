...
namespace sict {
	...
	Contact::Contact(const Contact &orgContact) {  //Copy Constructor

		if (!orgContact.isEmpty()) { //orginal contact is not empty 


			copyContact(orgContact);
	...
	void Contact::copyContact(const Contact &Source_Contact) {

		//Copy values of orgContact
		strcpy_s(mContact_Name, Source_Contact.mContact_Name);

		mNumOfPhoneNums = 0; //The NewPhoneNum() function handles adding +1 to mNUmOfPhoneNums for me

		//DEEP copy mPhone_Nums (in case the other object goes out of scope and mPhone_Nums gets deallocated)

		if (Source_Contact.mPhone_Nums != nullptr) { //if source's mPhone_Nums is not a nullptr then copy all numbers from soure contact
			
			for (int i = 0; i < Source_Contact.mNumOfPhoneNums; ++i) {
				NewPhoneNum(Source_Contact.mPhone_Nums[i]);
			}
	...
	Contact& Contact::operator=(const Contact &Source_Contact) {

		if (this != &Source_Contact) {

			copyContact(Source_Contact);
	...
	void Contact::operator+=(long long Phone_Num)
	{

		NewPhoneNum(Phone_Num);
